...
func main() {
	...
	if *samples <= 0 { fatal(errors.New("-samples must be positive")) }
	operators, err := loadOperators(*operatorsPath); if err != nil { fatal(err) }
	if len(operators) < 2 { fatal(errors.New("at least two operators are required")) }
	...
	for sequence := 1; sequence <= *samples; sequence++ {
		for _, operator := range operators { window.Samples = append(window.Samples, collect(client, sequence, operator)) }
		if sequence < *samples { time.Sleep(*interval) }
	}
	window.EndedAt = time.Now().UTC()
	for _, sample := range window.Samples {
		if sample.Checkpoint == nil { continue }
		window.NetworkID = sample.Checkpoint.NetworkID; window.GenesisHash = sample.Checkpoint.GenesisHash; window.CarrotPolicyHash = sample.Checkpoint.CarrotPolicyHash
		break
	}
	...
	if *gate && !readiness.Ready { fatal(fmt.Errorf("live evidence gate not ready: %s", strings.Join(readiness.Reasons,"; "))) }
}
...
func loadOperators(path string) ([]publictestnet.OperatorDescriptor,error) {
	...
	if err := json.Unmarshal(raw,&operators); err != nil { return nil,fmt.Errorf("decode operators: %w",err) }
	for i := range operators { operators[i].ID = strings.TrimSpace(operators[i].ID); operators[i].URL = strings.TrimRight(strings.TrimSpace(operators[i].URL),"/") }
...
func collect(client *http.Client, sequence int, operator publictestnet.OperatorDescriptor) publictestnet.LiveEvidenceSample {
	...
	started := time.Now(); status, err := getMap(client,operator.URL+"/v1/public/status"); sample.StatusLatencyMillis = time.Since(started).Milliseconds()
	if err != nil { sample.Error = "status: "+err.Error(); return sample }
	sample.Status = status
	started = time.Now(); checkpoint, err := getCheckpoint(client,operator.URL+"/v1/public/checkpoint"); sample.CheckpointLatencyMillis = time.Since(started).Milliseconds()
	if err != nil { sample.Error = "checkpoint: "+err.Error(); return sample }
	sample.Checkpoint = &checkpoint; sample.Available = true; return sample
}

func getMap(client *http.Client, url string) (map[string]any,error) {
	resp,err := client.Get(url); if err != nil { return nil,err }; defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK { body,_:=io.ReadAll(io.LimitReader(resp.Body,4096)); return nil,fmt.Errorf("status %d: %s",resp.StatusCode,strings.TrimSpace(string(body))) }
	var value map[string]any; if err := json.NewDecoder(io.LimitReader(resp.Body,1<<20)).Decode(&value); err != nil { return nil,err }; return value,nil
}

func getCheckpoint(client *http.Client, url string) (publictestnet.Checkpoint,error) {
	resp,err := client.Get(url); if err != nil { return publictestnet.Checkpoint{},err }; defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK { body,_:=io.ReadAll(io.LimitReader(resp.Body,4096)); return publictestnet.Checkpoint{},fmt.Errorf("status %d: %s",resp.StatusCode,strings.TrimSpace(string(body))) }
	var value publictestnet.Checkpoint; if err := json.NewDecoder(io.LimitReader(resp.Body,1<<20)).Decode(&value); err != nil { return value,err }; return value,nil
}

func fatal(err error) { fmt.Fprintln(os.Stderr,"pop-live-evidence:",err); os.Exit(1) }
