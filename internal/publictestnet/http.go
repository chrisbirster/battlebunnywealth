...
func (s *HTTPServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case r.Method == http.MethodGet && r.URL.Path == "/v1/public/funding-policy":
		policy := DefaultTestFundingPolicy()
	...
	case r.Method == http.MethodPost && r.URL.Path == "/v1/public/peers":
		...
		if err := s.Directory.Upsert(a, time.Now().UTC()); err != nil {
	...
	case r.Method == http.MethodPost && r.URL.Path == "/v1/public/transactions":
		...
		if err := s.Mempool.Admit(tx, nextHeight, s.Ledger.Nonce(tx.From)); err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
			return
		}
	...
	case r.Method == http.MethodGet && r.URL.Path == "/v1/public/ledger":
		...
		s.writeJSON(w, http.StatusOK, map[string]any{
			"account":      account,
			"balanceAtoms": s.Ledger.Balance(account),
			"nonce":        s.Ledger.Nonce(account),
			"supply":       s.Ledger.SupplyReport(),
		})
	case r.Method == http.MethodPost && r.URL.Path == "/v1/public/validators/apply":
		if s.Admission == nil {
			http.Error(w, "validator admission unavailable", http.StatusServiceUnavailable)
			return
		}
		...
		if err := s.Admission.Submit(r.Context(), a, time.Now().UTC()); err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
			return
		}
...
func (s *HTTPServer) currentHeight() uint64 {
	if s.Height == nil {
		return 0
	}
	return s.Height()
...
func (s *HTTPServer) handleStatus(w http.ResponseWriter) {
	peers := 0
	if s.Directory != nil {
		peers = len(s.Directory.List(time.Now().UTC()))
	}
	pending, active := 0, 0
	if s.Admission != nil {
		pending = s.Admission.Pending()
		active = len(s.Admission.Active())
	}
	mempoolCount := 0
	if s.Mempool != nil {
