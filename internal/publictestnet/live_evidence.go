...
func (w *LiveEvidenceWindow) Finalize() error {
	...
	for _, incident := range w.detectCheckpointIncidents() {
		if !w.hasIncident(incident.Kind, incident.Height) {
	...
	sort.Slice(w.Incidents, func(i, j int) bool {
		if w.Incidents[i].Height == w.Incidents[j].Height {
			if w.Incidents[i].Kind == w.Incidents[j].Kind {
				return w.Incidents[i].ObservedAt.Before(w.Incidents[j].ObservedAt)
			}
			return w.Incidents[i].Kind < w.Incidents[j].Kind
		}
		return w.Incidents[i].Height < w.Incidents[j].Height
	})
...
func (w LiveEvidenceWindow) Validate() error {
	...
	for _, incident := range w.detectCheckpointIncidents() {
		if !w.hasIncident(incident.Kind, incident.Height) {
			return fmt.Errorf("%w: missing recorded checkpoint incident for height %d", ErrInvalidLiveEvidence, incident.Height)
		}
	}
	return nil
}
...
func (w LiveEvidenceWindow) validate(requireHash bool) error {
	...
	for _, operator := range w.Operators {
		...
		if _, ok := operators[id]; ok {
			return fmt.Errorf("%w: duplicate operator %q", ErrInvalidLiveEvidence, id)
		}
		if _, ok := urls[url]; ok {
			return fmt.Errorf("%w: duplicate operator url %q", ErrInvalidLiveEvidence, url)
		}
...
func (w LiveEvidenceWindow) detectCheckpointIncidents() []EvidenceIncident {
	...
	latestAt := map[uint64]time.Time{}
	latestSequence := map[uint64]int{}
	for _, sample := range w.Samples {
		if !sample.Available || sample.Checkpoint == nil {
			continue
		}
		height := sample.Checkpoint.Height
		byHeight[height] = append(byHeight[height], CheckpointObservation{Source: fmt.Sprintf("%s/sample-%d", sample.OperatorID, sample.Sequence), Checkpoint: *sample.Checkpoint})
		if sample.CollectedAt.After(latestAt[height]) {
			latestAt[height] = sample.CollectedAt
			latestSequence[height] = sample.Sequence
		}
	}
	incidents := []EvidenceIncident{}
	for height, observations := range byHeight {
		if len(observations) < 2 {
			continue
		}
		if _, err := CompareCheckpointSet(observations); err != nil {
			incidents = append(incidents, EvidenceIncident{Kind: IncidentCheckpointMismatch, ObservedAt: latestAt[height], Sequence: latestSequence[height], Height: height, Details: err.Error()})
		}
	}
	return incidents
}

func (w LiveEvidenceWindow) hasIncident(kind string, height uint64) bool {
	for _, incident := range w.Incidents {
		if incident.Kind == kind && incident.Height == height {
			return true
		}
	}
	return false
...
func (w LiveEvidenceWindow) EvaluateReadiness(policy EvidenceReadinessPolicy) EvidenceReadiness {
	if policy.MinOperators <= 0 {
		policy.MinOperators = 2
	}
	if policy.MinProviders <= 0 {
		policy.MinProviders = 2
	}
	if policy.MinRegions <= 0 {
		policy.MinRegions = 1
	}
	if policy.MinSamplesPerOperator <= 0 {
		policy.MinSamplesPerOperator = 1
	}
	providers := map[string]struct{}{}
	regions := map[string]struct{}{}
	counts := map[string]int{}
	for _, operator := range w.Operators {
		if value := strings.ToLower(strings.TrimSpace(operator.Provider)); value != "" {
			providers[value] = struct{}{}
		}
		if value := strings.ToLower(strings.TrimSpace(operator.Region)); value != "" {
			regions[value] = struct{}{}
		}
	}
	for _, sample := range w.Samples {
		if sample.Available {
			counts[sample.OperatorID]++
		}
	}
	minSamples := 0
	for _, operator := range w.Operators {
	...
	if result.Operators < policy.MinOperators {
		result.Reasons = append(result.Reasons, fmt.Sprintf("operators=%d want>=%d", result.Operators, policy.MinOperators))
	}
	if result.Providers < policy.MinProviders {
		result.Reasons = append(result.Reasons, fmt.Sprintf("providers=%d want>=%d", result.Providers, policy.MinProviders))
	}
	if result.Regions < policy.MinRegions {
		result.Reasons = append(result.Reasons, fmt.Sprintf("regions=%d want>=%d", result.Regions, policy.MinRegions))
	}
	if result.MinSamplesObserved < policy.MinSamplesPerOperator {
		result.Reasons = append(result.Reasons, fmt.Sprintf("availableSamplesPerOperator=%d want>=%d", result.MinSamplesObserved, policy.MinSamplesPerOperator))
	}
	if policy.MinWindow > 0 && w.EndedAt.Sub(w.StartedAt) < policy.MinWindow {
		result.Reasons = append(result.Reasons, fmt.Sprintf("window=%s want>=%s", w.EndedAt.Sub(w.StartedAt), policy.MinWindow))
	}
	for _, incident := range w.Incidents {
		if incident.Kind == IncidentCheckpointMismatch {
			result.Reasons = append(result.Reasons, fmt.Sprintf("consensus incident at height %d: %s", incident.Height, incident.Details))
		}
	}
