...
func TestLiveEvidenceFinalizesAndValidates(t *testing.T) {
	start := time.Unix(1_800_000_000,0).UTC(); checkpoint := liveEvidenceCheckpoint("network","genesis","policy",42,"block","state")
	...
	if err := window.Validate(); err != nil { t.Fatal(err) }
	...
	if !readiness.Ready { t.Fatalf("readiness=%+v", readiness) }
}

func TestLiveEvidencePreservesSameHeightDisagreement(t *testing.T) {
	start := time.Unix(1_800_000_000,0).UTC(); a := liveEvidenceCheckpoint("network","genesis","policy",42,"block-a","state-a"); b := liveEvidenceCheckpoint("network","genesis","policy",42,"block-b","state-b")
	...
	if err := window.Finalize(); err != nil { t.Fatal(err) }
	if err := window.Validate(); err != nil { t.Fatal(err) }
	if len(window.Incidents) != 1 || window.Incidents[0].Kind != IncidentCheckpointMismatch { t.Fatalf("incidents=%+v", window.Incidents) }
	if readiness := window.EvaluateReadiness(EvidenceReadinessPolicy{MinOperators:2,MinProviders:2}); readiness.Ready { t.Fatalf("unexpected readiness=%+v", readiness) }
}

func TestLiveEvidenceAllowsHeightLag(t *testing.T) {
	start := time.Unix(1_800_000_000,0).UTC(); a := liveEvidenceCheckpoint("network","genesis","policy",42,"block-a","state-a"); b := liveEvidenceCheckpoint("network","genesis","policy",41,"block-b","state-b")
	...
	if err := window.Finalize(); err != nil { t.Fatal(err) }
	if len(window.Incidents) != 0 { t.Fatalf("incidents=%+v", window.Incidents) }
}
