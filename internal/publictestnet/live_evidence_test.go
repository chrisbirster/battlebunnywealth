...
func TestLiveEvidenceFinalizesAndValidates(t *testing.T) {
	...
	window := LiveEvidenceWindow{
		Version: LiveEvidenceVersion, RepositorySHA: "0123456789012345678901234567890123456789", NetworkID: "network", GenesisHash: "genesis", CarrotPolicyHash: "policy", NetworkMapHash: "map", StartedAt: start, EndedAt: start.Add(time.Hour),
		Operators: []OperatorDescriptor{{ID:"a",URL:"https://a.example",Provider:"provider-a",Region:"us-east"},{ID:"b",URL:"https://b.example",Provider:"provider-b",Region:"us-west"}},
		Samples: []LiveEvidenceSample{{Sequence:1,CollectedAt:start.Add(time.Minute),OperatorID:"a",Available:true,Checkpoint:&checkpoint},{Sequence:1,CollectedAt:start.Add(2*time.Minute),OperatorID:"b",Available:true,Checkpoint:&checkpoint}},
	}
	if err := window.Finalize(); err != nil { t.Fatal(err) }
	if window.Hash == "" { t.Fatal("missing evidence hash") }
...
func TestLiveEvidenceRejectsSameHeightDisagreement(t *testing.T) {
	start := time.Unix(1_800_000_000, 0).UTC()
	...
	if err := window.Finalize(); !errors.Is(err, ErrLiveConsensusMismatch) { t.Fatalf("err=%v", err) }
}
