...
const (
	...
	AttestationPurposeEnrollment = "enrollment"
	AttestationPurposeAssertion  = "assertion"
...
type AttestationChallenge struct {
	...
	AssertionPurpose    string     `json:"assertionPurpose,omitempty"`
...
type DeviceAttestationRecord struct {
	...
	ProviderPublicKeySPKI string    `json:"providerPublicKeySpki,omitempty"`
...
type AttestationProviderVerifier interface {
	Provider() string
	VerifyEnrollment(context.Context, AttestationChallenge, AttestationEvidence) (AttestationResult, error)
}
...
func NewAttestationService(now func() time.Time, store AttestationStore, config AttestationConfig, providers ...AttestationProviderVerifier) (*AttestationService, error) {
	...
	if state.SchemaVersion != AttestationSchemaVersion {
...
func (s *AttestationService) Begin(accountID, deviceID, publicKeySPKI, provider string) (AttestationChallenge, error) {
	...
	if !validDevicePublicKey(publicKeySPKI) {
		return AttestationChallenge{}, ErrAttestationBinding
	}
	now := s.now().UTC()
	for i := range s.state.Challenges {
		c := &s.state.Challenges[i]
		if c.AccountID == accountID && c.DeviceID == deviceID && c.Provider == provider && attestationPurpose(c.Purpose) == AttestationPurposeEnrollment && c.UsedAt == nil && c.ExpiresAt.After(now) {
			return publicAttestationChallenge(*c), nil
		}
	}
	...
	keyHash := sha256.Sum256([]byte(publicKeySPKI))
	keyHashText := base64.RawURLEncoding.EncodeToString(keyHash[:])
	binding := fmt.Sprintf("bbw-attestation/v1\nchallenge=%s\ndevice=%s\npublicKeySha256=%s", nonce, deviceID, keyHashText)
	requestHashBytes := sha256.Sum256([]byte(binding))
	requestHash := base64.RawURLEncoding.EncodeToString(requestHashBytes[:])
	challenge := AttestationChallenge{
		ID: id, AccountID: accountID, DeviceID: deviceID, Provider: provider,
		Purpose: AttestationPurposeEnrollment, Nonce: nonce, DevicePublicKeyHash: keyHashText,
		BindingPayload: binding, RequestHash: requestHash, IssuedAt: now, ExpiresAt: now.Add(s.config.ChallengeTTL),
	}
...
func (s *AttestationService) Complete(ctx context.Context, accountID, deviceID string, evidence AttestationEvidence) (DeviceAttestationRecord, error) {
	...
	if attestationPurpose(challenge.Purpose) != AttestationPurposeEnrollment || challenge.AccountID != accountID || challenge.DeviceID != deviceID || evidence.DeviceID != deviceID || evidence.Provider != challenge.Provider {
		s.mu.Unlock()
	...
	if err == nil && result.ProviderPublicKeySPKI != "" {
		for _, existing := range s.state.Devices {
			if existing.Provider == challenge.Provider && existing.ProviderPublicKeySPKI == result.ProviderPublicKeySPKI && (existing.AccountID != accountID || existing.DeviceID != deviceID) {
				err = errors.New("attested provider public key is already associated with another device")
				break
			}
		}
	}
	record := DeviceAttestationRecord{
		AccountID: accountID, DeviceID: deviceID, Provider: challenge.Provider,
		Status: AttestationStateRejected, VerifiedAt: verifiedAt, Reason: "provider verification failed",
	}
	if err == nil {
		record.Status = AttestationStateVerified
		record.ProviderKeyID = result.ProviderKeyID
		record.ProviderPublicKeySPKI = result.ProviderPublicKeySPKI
	...
	if saveErr := s.store.Save(s.state); saveErr != nil {
		return DeviceAttestationRecord{}, saveErr
	}
	if err != nil {
		return record, fmt.Errorf("%w: %v", ErrAttestationRejected, err)
	}
	return record, nil
}
...
func attestationPurpose(purpose string) string {
	if purpose == "" {
		return AttestationPurposeEnrollment
	}
	return purpose
}

func publicAttestationChallenge(c AttestationChallenge) AttestationChallenge {
	c.AccountID = ""
	return c
}

func attestationToken(n int) (string, error) {
	b := make([]byte, n)
