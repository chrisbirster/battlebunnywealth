...
func testDeviceSPKI(t *testing.T)string{t.Helper();key,err:=ecdsa.GenerateKey(elliptic.P256(),rand.Reader);if err!=nil{t.Fatal(err)};raw,err:=x509.MarshalPKIXPublicKey(&key.PublicKey);if err!=nil{t.Fatal(err)};return base64.RawURLEncoding.EncodeToString(raw)}

func TestAttestationChallengeSingleUseAndBinding(t *testing.T){
	now:=time.Date(2026,9,12,12,0,0,0,time.UTC);clock:=func()time.Time{return now};store:=NewMemoryAttestationStore();svc,err:=NewAttestationService(clock,store,DefaultAttestationConfig(),DevelopmentAttestationVerifier{});if err!=nil{t.Fatal(err)};key:=testDeviceSPKI(t)
	challenge,err:=svc.Begin("acct","dev",key,ProviderDevelopment);if err!=nil{t.Fatal(err)}
	if challenge.RequestHash==""||challenge.DevicePublicKeyHash==""||challenge.BindingPayload==""{t.Fatal("expected key-bound challenge")}
...
func TestAttestationChallengeExpires(t *testing.T){
	now:=time.Date(2026,9,12,12,0,0,0,time.UTC);clock:=func()time.Time{return now};svc,_:=NewAttestationService(clock,NewMemoryAttestationStore(),AttestationConfig{ChallengeTTL:time.Minute},DevelopmentAttestationVerifier{});challenge,_:=svc.Begin("acct","dev",testDeviceSPKI(t),ProviderDevelopment);now=now.Add(2*time.Minute)
...
func TestAppleAdapterRequiresHardwareBackedResult(t *testing.T){
	...
	result,err:=verifier.VerifyEnrollment(context.Background(),AttestationChallenge{Nonce:"nonce",RequestHash:"hash"},AttestationEvidence{KeyID:"key",Payload:"object"});if err!=nil{t.Fatal(err)}
...
func TestPlayIntegrityRejectsWrongRequestHash(t *testing.T){
	...
	_,err:=verifier.VerifyEnrollment(context.Background(),AttestationChallenge{ID:"c",DeviceID:"d",Nonce:"n",RequestHash:"right"},AttestationEvidence{Payload:"token"});if err==nil{t.Fatal("expected rejection")}
}

func TestDeviceWeights(t *testing.T){want:=[]int{100,25,10,2,2};for i,w:=range want{if got:=DeviceWeightPercent(i);got!=w{t.Fatalf("ordinal %d=%d want %d",i,got,w)}}}
