...
type AttestationVerifier interface {
	Verify(participant Participant, challenge Challenge, proof ParticipationProof) error
}
...
func NewProtocol(config Config, chain *Chain) *Protocol {
	return &Protocol{config: config, chain: chain, started: time.Now().UTC()}
}

func (p *Protocol) CurrentEpoch(now time.Time) uint64 {
	if now.Before(p.started) {
		return 0
	}
	return uint64(now.Sub(p.started) / p.config.EpochDuration)
}
...
func ValidateEnvelope(now time.Time, participant Participant, challenge Challenge, proof ParticipationProof) error {
	...
	if proof.ParticipantID != participant.ID {
		return ErrParticipantMismatch
	}
	if proof.DevicePublicKey != participant.DevicePublicKey {
