...
func (n *HTTPNode) enter(r *http.Request) bool {
	...
	n.mu.Lock()
	defer n.mu.Unlock()
	if n.inFlight[host] >= n.cfg.MaxConcurrentRequestsPerIP {
		return false
	}
	n.inFlight[host]++
	return true
...
func (n *HTTPNode) leave(r *http.Request) {
	...
	defer n.mu.Unlock()
	n.inFlight[host]--
	if n.inFlight[host] <= 0 {
		delete(n.inFlight, host)
	}
}
...
func (n *HTTPNode) handleDraft(w http.ResponseWriter, r *http.Request) {
	...
	round64, _ := strconv.ParseUint(r.URL.Query().Get("round"), 10, 32)
	block, err := n.Engine.Draft(uint32(round64), validator, nil, time.Now())
	if err != nil {
		n.error(w, err)
		return
	}
...
func (n *HTTPNode) handleDirectProposal(w http.ResponseWriter, r *http.Request) {
	...
	n.bump(true)
	if n.Relayer != nil {
		_ = n.Relayer.Broadcast(r.Context(), "proposal", p)
	}
...
func (n *HTTPNode) handleDirectVote(w http.ResponseWriter, r *http.Request) {
	...
	if finalized && n.Store != nil {
		blocks := n.Engine.Finalized()
		if err := n.Store.Append(blocks[len(blocks)-1]); err != nil {
	...
	n.bump(true)
	if n.Relayer != nil {
		_ = n.Relayer.Broadcast(r.Context(), "vote", v)
	}
...
func (n *HTTPNode) handlePeer(w http.ResponseWriter, r *http.Request, kind string) {
	if n.Guard == nil {
		http.Error(w, "peer ingress disabled", http.StatusForbidden)
		return
	}
	var e PeerEnvelope
	if !n.decode(w, r, &e) {
	...
	if e.Kind != kind {
		n.error(w, errors.New("peer message kind mismatch"))
		return
	}
	if err := n.Guard.Verify(e, time.Now().UTC()); err != nil {
		n.error(w, err)
		return
	}
	switch kind {
	case "proposal":
		...
		if json.Unmarshal(e.Payload, &p) != nil {
			n.error(w, errors.New("invalid proposal payload"))
			return
		}
		if err := n.Engine.HandleProposal(p); err != nil {
			n.error(w, err)
	...
	case "vote":
		...
		if json.Unmarshal(e.Payload, &v) != nil {
			n.error(w, errors.New("invalid vote payload"))
			return
		}
		finalized, err := n.Engine.HandleVote(v)
		if err != nil {
...
func (n *HTTPNode) handleBlocks(w http.ResponseWriter, r *http.Request) {
	...
	if from > uint64(len(all))+1 {
		http.Error(w, "from height beyond head", http.StatusBadRequest)
		return
	}
	if from > uint64(len(all)) {
		n.writeJSON(w, http.StatusOK, []FinalizedBlock{})
		return
	}
	n.writeJSON(w, http.StatusOK, all[from-1:])
}
func (n *HTTPNode) handleMetrics(w http.ResponseWriter) {
	metrics := map[string]any{"status": n.Engine.Status(), "acceptedMessages": n.accepted, "rejectedMessages": n.rejected}
	if n.Guard != nil {
		metrics["peers"] = n.Guard.Metrics()
	}
	n.writeJSON(w, http.StatusOK, metrics)
}
func (n *HTTPNode) bump(ok bool) {
	n.mu.Lock()
	defer n.mu.Unlock()
	if ok {
		n.accepted++
...
func (n *HTTPNode) error(w http.ResponseWriter, err error) {
	n.bump(false)
	status := http.StatusBadRequest
	if errors.Is(err, ErrUnknownPeer) || errors.Is(err, ErrInvalidSignature) {
		status = http.StatusForbidden
	}
	http.Error(w, err.Error(), status)
}
func (n *HTTPNode) writeJSON(w http.ResponseWriter, status int, v any) {
...
func SyncFrom(ctxURL string, client *http.Client, engine *Engine) error {
	...
	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("genesis status %d", resp.StatusCode)
	}
	var remote Genesis
	if err := json.NewDecoder(io.LimitReader(resp.Body, 1<<20)).Decode(&remote); err != nil {
		return err
	}
	if remote.Hash != engine.Genesis().Hash {
		return errors.New("peer genesis mismatch")
	}
	...
	if resp2.StatusCode != http.StatusOK {
		return fmt.Errorf("blocks status %d", resp2.StatusCode)
	}
	var blocks []FinalizedBlock
	if err := json.NewDecoder(io.LimitReader(resp2.Body, 8<<20)).Decode(&blocks); err != nil {
		return err
	}
